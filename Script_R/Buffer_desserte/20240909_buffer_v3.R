...
# Charger les données de routes et les convertir en objet sf si nécessaire
...
# Correction des superpositions avec st_difference
...
# Convertir en objets sf avec la colonne geometry
...
# Combiner tous les buffers en une seule couche
...
# Visualiser les buffers avec des couleurs monochromes et ajustements
tm_shape(Saxel_parca_pci) +
  tm_borders() +  # Ajouter les frontières de la parcelle
  tm_shape(buffers_combined) +
  tm_fill(col = "Buffer", 
          palette = "-Blues",  # Palette monochrome bleue
          alpha = 0.2,  # Transparence ajustée pour voir le parcellaire
  ...
  tm_layout(legend.outside = TRUE, 
            legend.outside.position = "right",  # Légende à droite
            legend.title.size = 1.5,            # Taille du titre de la légende
            legend.text.size = 1.2,             # Taille du texte de la légende
            legend.bg.color = "white",          # Fond blanc pour la légende
            legend.bg.alpha = 0.8)              # Transparence du fond de légende
